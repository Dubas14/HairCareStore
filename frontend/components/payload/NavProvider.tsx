...
const NavProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  useEffect(() => {
    ...
    /** Inject theme toggle button into nav controls (idempotent) */
    function injectThemeToggle() {
      const aside = document.querySelector('aside.nav')
      if (!aside) return
      if (aside.querySelector('.hl-theme-toggle')) return

      const controls = aside.querySelector('.nav__controls')
      if (!controls) return
      ...
      btn.className = 'hl-theme-toggle'
      btn.type = 'button'
      btn.title = 'Перемкнути тему'
      ...
      btn.addEventListener('click', () => {
        ...
        // Set cookie so Payload's ThemeProvider persists the choice
        const d = new Date()
        d.setTime(d.getTime() + 365 * 24 * 60 * 60 * 1000)
        document.cookie = `payload-theme=${next};expires=${d.toUTCString()};path=/`
        updateIcon()
      })

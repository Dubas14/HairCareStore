...
export async function generateStaticParams() {
  ...
  // Try to get additional pages from Strapi
  try {
    const pages = await getPages()
    const strapiSlugs = pages.map(page => page.slug)

    // Combine and deduplicate
    const allSlugs = [...new Set([...knownSlugs, ...strapiSlugs])]
    return allSlugs.map(slug => ({ slug }))
  } catch {
    // If Strapi is unavailable, use known slugs only
...
export default async function StaticPage({ params }: PageProps) {
  ...
  // For known page types, we can render without Strapi data
  // For generic pages, we need Strapi content
  if (!page && pageType === 'generic') {
    notFound()
  }

  // Use Strapi title if available, otherwise use fallback
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 md:py-16">
        ...
        {/* For generic pages, render Strapi content */}
